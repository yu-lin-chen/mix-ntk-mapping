...
struct choice_lut_map_params
{
  ...
  {
    cut_enumeration_ps.cut_size = 4u;
  ...
  uint32_t sop_cut_limit{ 10 };
   std::function<void(uint32_t, std::function<void(uint32_t)> const&)> cross_choice_provider{};
};
...
auto has_foreach_choice_impl( int ) -> decltype( std::declval<T&>().foreach_choice( uint32_t{}, std::declval<Fn>() ), std::true_type{} );

template<typename, typename>
auto has_foreach_choice_impl( ... ) -> std::false_type;
template<typename T, typename Fn>
...
void choose_best_po()
{
  ...
  std::cout << "\n[choose_best_po] start\n";
  std::cout << "  num_pos(before) = " << ntk.num_pos() << "\n";

  // --- 评分函数：从 PO root DFS 统计唯一 cut 数 ---
  auto score_from_po = [&]( uint32_t root_idx ) -> uint32_t {
    std::unordered_set<uint32_t> visited;
    std::function<uint32_t(uint32_t)> dfs = [&]( uint32_t idx ) -> uint32_t {
      if ( idx == 0 || ntk.is_ci( ntk.index_to_node( idx ) ) ||
           ntk.is_constant( ntk.index_to_node( idx ) ) )
        return 0;

      if ( !visited.insert( idx ).second )
        return 0;

      auto const& best_cut = cuts[idx][0];
      uint32_t cnt = 1;
  ...
  // --- 选每个 PO 的最优等价节点 ---
  std::vector<signal_t> best_po_signals;
  best_po_signals.reserve( ntk.num_pos() );

  ntk.foreach_po( [&]( auto const& f, auto po_idx ) {
    auto root_idx = ntk.node_to_index( ntk.get_node( f ) );
    bool po_compl = ntk.is_complemented( f );

    std::vector<uint32_t> candidates;
    candidates.push_back( root_idx );

    if constexpr ( has_foreach_choice_v<Ntk, std::function<void(uint32_t)>> )
    {
      ntk.foreach_choice( root_idx, [&]( uint32_t cand_idx ) {
        if ( cand_idx != root_idx )
          candidates.push_back( cand_idx );
      } );
    }

    std::cout << "  PO[" << po_idx << "] root=" << root_idx
              << " compl=" << po_compl
              << " candidates={";
    for ( auto c : candidates ) std::cout << c << " ";
    std::cout << "}\n";

    uint32_t best_idx = candidates.front();
    uint32_t best_score = score_from_po( best_idx );

    std::cout << "    score(" << best_idx << ")=" << best_score << "\n";
    for ( auto cand : candidates )
    {
      auto score = score_from_po( cand );
      std::cout << "    score(" << cand << ")=" << score << "\n";
      if ( score < best_score )
      {
        best_score = score;
        best_idx = cand;
  ...
  std::function<void(uint32_t)> mark_refs = [&]( uint32_t idx ) {
    if ( idx == 0 || ntk.is_ci( ntk.index_to_node( idx ) ) ||
         ntk.is_constant( ntk.index_to_node( idx ) ) )
      return;

    if ( node_match[idx].map_refs++ > 0 )
      return;
  ...
  ntk.foreach_po( [&]( auto const& f, auto po_idx ) {
    auto idx = ntk.node_to_index( ntk.get_node( f ) );
    std::cout << "  mark_refs from PO[" << po_idx << "] root=" << idx << "\n";
    mark_refs( idx );
  } );

  std::cout << "  map_refs nonzero nodes: ";
  for ( uint32_t i = 0; i < node_match.size(); ++i )
  {
    if ( node_match[i].map_refs > 0 )
      std::cout << i << "(" << node_match[i].map_refs << ") ";
  }
  std::cout << "\n[choose_best_po] end\n";
}
  void perform_mapping()
  {
    ...
    lut_cut_sort_type area_sort =
        ( ps.area_oriented_mapping && !ps.edge_optimization )//area=false/edge=true
  ...
  template<bool DO_AREA, bool ELA>//false
                        ...
                        bool recompute_cuts )
  {
    ...
    for ( auto const& n : topo_order )
    {
      ...
      if ( recompute_cuts )//true
      {
        if constexpr ( Ntk::min_fanin_size == 2 && Ntk::max_fanin_size == 2 )//false
        {
 ...
 template<bool DO_AREA, bool ELA>
  void compute_choice_cut( node const& n, lut_cut_sort_type const sort, bool preprocess )
  {
    (void)DO_AREA; (void)ELA; (void)preprocess;
...
if constexpr ( has_foreach_choice_v<Ntk, std::function<void(uint32_t)>> )
{
  ntk.foreach_choice( ntk.node_to_index( n ), [&]( uint32_t cand_index ) {
      if ( cuts[cand_index].size() == 0 ) return;

      for ( auto* cut : cuts[cand_index] )
      {
    ...
    cuts[index].limit( ps.cut_enumeration_ps.cut_limit );
    cuts[index].replace( 0, best_cut );
    add_unit_cut( index );
    cut_ref( cuts[index].best() );
  }
  
const char* gate_type_name( node const& n ) const
{
  if ( ntk.is_and( n ) ) return "and2";
  if ( ntk.is_or ( n ) ) return "or2";
  if ( ntk.is_xor( n ) ) return "xor2";
  if ( ntk.is_maj( n ) ) return "maj3";
  if ( ntk.is_xor3( n ) ) return "xor3";
  if ( ntk.is_ite( n ) ) return "ite3";
  if ( ntk.is_pi( n ) ) return "pi";
  if ( ntk.is_constant( n ) ) return "const";
  return "unknown";
}

void debug_print_cut( cut_t const& c, uint32_t index, const char* tag = "" ) const
{
  auto n = ntk.index_to_node( index );
  std::cout << "\n=== best cut " << tag << " (node index = " << index
            << ", type=" << gate_type_name( n ) << ") ===\n";
  std::cout << "size=" << c.size();
  if constexpr ( StoreFunction )
    std::cout << " func_id=" << c->func_id;
  std::cout << " delay=" << c->data.delay
            << " area=" << c->data.lut_area << "\n  leaves: ";
  for ( auto l : c )
    std::cout << l << " ";
  std::cout << "\n===========================================\n";
}
                         ...
                         bool preprocess )
  {
      ...
      foreach_mixed_radix_tuple(
          cut_sizes.begin(), cut_sizes.end(), [&]( auto begin, auto end )
          {
            ...
            for (i = 2; i < fanin; ++i) {
              ...
              //until there is correct
            if (ps.remove_dominated_cuts && rcuts.is_dominated(new_cut)) {
              return true; /* continue */
            }
            if constexpr (StoreFunction) {//true
              new_cut->func_id = compute_truth_table(index, vcuts, new_cut);
            }
    ...
    // ... compute_best_cut 末尾
 // debug_print_cut( rcuts[0], index, "final" );
